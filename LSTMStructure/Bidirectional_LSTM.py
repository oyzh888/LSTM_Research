...
exp_name = 'BLSTM_LayerN%d_LS%d_WinSize%d_BS%d_%dpart_epoch%d' % (LSTM_Layer_Num, LayerSize, maxlen, batch_size, how_much_part, epochs)
log = '../res/' + exp_name + '.txt'
tb_log = '../TB_logdir/LSTM/RNNStructure_6_30/' + exp_name
...
if(LSTM_Layer_Num > 1):
    ...
    for i in range(LSTM_Layer_Num - 1):
        if(i == LSTM_Layer_Num - 2):
            xxx = Bidirectional(LSTM(LayerSize, return_sequences=False,dropout=0))(xxx)
        else:
            xxx = Bidirectional(LSTM(LayerSize, return_sequences=True,dropout=0))(xxx)
else:
    xxx = Bidirectional(LSTM(LayerSize, return_sequences=False))(inputs)
